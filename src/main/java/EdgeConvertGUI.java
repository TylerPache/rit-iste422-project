...
public class EdgeConvertGUI {
   ...
   public EdgeConvertGUI() {
      ...
      this.showGUI();
      logger.debug("Creating EdgeConvertGUI object.");
   } // EdgeConvertGUI.EdgeConvertGUI()

   public void showGUI() {
      try {
         logger.info("Showing edge converter GUI...");
   ...
   public void createDTScreen() {// create Define Tables screen
      ...
      jbDTDefaultValue.addActionListener(new ActionListener() {
         public void actionPerformed(ActionEvent ae) {
            ...
            do {
               String result = (String) JOptionPane.showInputDialog(null, "Enter the default value:", "Default Value",
                     JOptionPane.PLAIN_MESSAGE, null, null, prev);
               ...
               case 0: // varchar
                  if (result.length() <= Integer.parseInt(jtfDTVarchar.getText())) {
                     ...
                     logger.warn("\"" + result + "\" has a value more than the varchar length specified.");
                  }
                  break;
               case 1: // boolean
                  ...
                  if (newResult.equals("true") || newResult.equals("false")) {
                     ...
                     logger.warn("\"" + result + "\" is not a valid boolean value.");
                  }
                  break;
               case 2: // Integer
                  try {
               ...
               case 4: // Timestamp
                  try {
                     ...
                     logger.warn(
                           "\"" + result + "\" is not a timestamp or is outside the bounds of valid timestamp values.");
                  }
                  break;
               }
            } while (!goodData);
      ...
      jbDTVarchar.addActionListener(new ActionListener() {
         public void actionPerformed(ActionEvent ae) {
            ...
            String result = (String) JOptionPane.showInputDialog(null, "Enter the varchar length:", "Varchar Length",
                  JOptionPane.PLAIN_MESSAGE, null, null, prev);
            if ((result == null)) {
            ...
            try {
               ...
               logger.warn("\"" + result + "\" is not a valid number.");
               jtfDTVarchar.setText(Integer.toString(EdgeField.VARCHAR_DEFAULT_LENGTH));
               return;
            }
            dataSaved = false;
         ...
         public void actionPerformed(ActionEvent ae) {
            ...
            if (currentDRField1.getFieldBound() == relatedField) { // the selected fields are already bound to each
               ...
               int answer = JOptionPane.showConfirmDialog(null,
                     "Do you wish to unbind the relation on field " + currentDRField1.getName() + "?", "Are you sure?",
            ...
            if (currentDRField1.getFieldBound() != 0) { // field is already bound to a different field
               int answer = JOptionPane.showConfirmDialog(null, "There is already a relation defined on field "
                     + currentDRField1.getName() + ", do you wish to overwrite it?", "Are you sure?",
                     JOptionPane.YES_NO_OPTION);
            ...
            JOptionPane.showMessageDialog(null,
                  "Table " + currentDRTable1.getName() + ": native field " + currentDRField1.getName()
                        + " bound to table " + currentDRTable2.getName() + " on field " + currentDRField2.getName());
            dataSaved = false;
         }
      });
   ...
   private void getOutputClasses() {
      ...
      try {
         ...
         logger.error("NoSuchMethodException: " + nsme);
   ...
   class EdgeWindowListener implements WindowListener {
      ...
      public void windowClosing(WindowEvent we) {
         if (!dataSaved) {
            int answer = JOptionPane.showOptionDialog(null, "You currently have unsaved data. Would you like to save?",
                  "Are you sure?", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, null, null);
            if (answer == JOptionPane.YES_OPTION) {
               if (saveFile == null) {
                  saveAs();
               }
   ...
   class CreateDDLButtonListener implements ActionListener {
      public void actionPerformed(ActionEvent ae) {
         ...
         try {
            ...
            JOptionPane.showMessageDialog(null, "Please try again with a valid path.");
         }
      }
   }
